...
// W1: 本地脚本路径支持环境变量覆盖；默认指向统一工作区中的排课系统。
export const SCHEDULE_SCRIPT = process.env.LAOSU_SCHEDULE_SCRIPT
  || "~/laosu-workspace/库管理与索引/老苏排课系统/schedule.py";
...
export const CALENDAR_SYNC_SCRIPT = process.env.LAOSU_CALENDAR_SYNC_SCRIPT
  || "~/laosu-workspace/库管理与索引/老苏排课系统/sync_to_calendar.py";
...
export async function dataRevision() {
  ...
  const candidates = [
    ...
    process.env.LAOSU_AFFAIRS_DB || "~/Shared/Hana主动式事务系统/data/production/affairs.db",
  ];
  let latest = 0;
...
// 纯函数：生成总览行动卡。required 表示欠老苏一个明确决定；suggestion 仅表示
// 可选的系统优化。事务 pending 已在左侧主队列展示，这里不重复生成同一事项。
export function buildSuggestionCards({ events, reviews, pending, calendar, conflicts, missingEnd, localDate }) {
  ...
  const push = (card) => {
    ...
    seen.add(card.id);
    cards.push({ rank: 100, bucket: "suggestion", ...card });
  };
  ...
  // pending_confirmation / postponed / needs_reschedule 已由事务 pending 主队列展示；
  // 到期预约、过期 deadline 仍是同一条事务，不在建议区复制第二张卡。
  void pending;
  ...
  // 明日课表已经在下一节课和周/月视图出现，属于信息，不占用“处理”队列。
  return cards.sort((a, b) => (a.bucket === b.bucket ? a.rank - b.rank : a.bucket === "required" ? -1 : 1));
}
...
// 结束时间只影响仍在推进的定时事务；已完成/已取消属于历史留痕，不应再冒充待处理建议。
export function filterMissingEndAffairs(affairs) {
  const openStatuses = new Set(["pending_confirmation", "scheduled", "in_progress", "postponed", "needs_reschedule"]);
  return (affairs ?? [])
    .filter((item) => item.kind === "timed" && item.start_at && !item.end_at && openStatuses.has(item.status))
    .map((item) => ({ id: item.id, title: item.title, start_at: item.start_at, status: item.status }));
}

// 数据质量自检：只为仍在推进的本月定时事务建议补结束时间。
export async function getAffairsMissingEnd() {
  ...
  return {
    ok: timeline.ok,
    month,
    items: filterMissingEndAffairs(affairs),
  };
}
