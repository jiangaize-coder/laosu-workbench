...
export async function getRuntimeVersions() {
  ...
  const devShadowing = Boolean(devVersion && installedVersion && devVersion !== installedVersion);
  return {
    ok: true,
    ...
    note: devShadowing
      ...
      : devVersion
        ? `开发槽 v${devVersion} 与正式版一致`
        : "无开发槽，正式版为运行版本",
  };
}

export async function getCalendarHealth() {
  // 阶段6（2026-08-26）：飞书日历同步差异检查——不写日历，只跑 dry-run 盘点。
  const result = await runCalendarSync(["--dry-run"]);
  ...
  const inSync = calendarEvents !== null && desiredEvents !== null && calendarEvents === desiredEvents && deleted.length === 0 && created.length === 0;
  return {
    ok: result.ok !== false,
    observedAt: new Date().toISOString(),
    diff: {
...
export function buildOperation(input = {}, dryRun = true) {
  ...
  if (operation === "alias_add") {
    const student = requireText(input.student, "学生");
    const alias = requireText(input.alias, "别名");
    const kind = input.kind && String(input.kind).trim() ? String(input.kind).trim() : "错别字";
